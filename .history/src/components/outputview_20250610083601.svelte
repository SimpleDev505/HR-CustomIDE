<script lang="ts">
  ...
  export async function FetchOuput() {
    ...
    if (LanguageId === null) {
      let statusMessage = `Compilation failed: Language '${Language}' not supported for compilation.`;
    ...
    try {
      ...
      let errorMsgs = []; // This is `string[]` and does NOT have an `.update()` method.
      ...
      // Correct usage: compiledOutputs.update()
      ...
      let errorMessage = `Judge0 API failed: ${error.message || error}`;
      ...
      compiledOutputs.update((outputs) => {
        outputs[currentEntryIndex] = {
          ...
          error: [errorMessage], // Corrected: Wrapped in array
