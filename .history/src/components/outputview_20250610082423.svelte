<script lang="ts">
  ...
  function GetLanguageId(lang: String) {
    switch (lang) {
      case "javascript":
        return 93; // JavaScript (Node.js 20.17.0)
      case "python":
        return 95; // Python (3.12.5)
      default:
        return null;
    }
  }
  export async function FetchOuput() {
    ...
    const monacoLanguageId = GetLanguageExtension();
    const judge0LanguageId = getJudge0LanguageId(monacoLanguageId);
    const options = {
      method: "POST",
