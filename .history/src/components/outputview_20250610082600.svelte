<script lang="ts">
  ...
  export async function FetchOuput() {
    ...
    if (judge0LanguageId === null) {
      statusMessage = `Compilation failed: Language '${monacoLanguageId}' not supported by Judge0 for direct compilation.`;
      compiledOutputs.update((outputs) => [
        ...outputs,
        {
          ...
          status: "error",
          output: "N/A",
          error: `Language '${monacoLanguageId}' cannot be compiled by Judge0.`,
        },
      ]);
      setTimeout(() => {
        statusMessage = "";
