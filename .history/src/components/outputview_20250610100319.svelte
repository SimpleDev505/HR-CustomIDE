<script lang="ts">
  ...
  async function FetchOuput() {
    ...
    const normalizedPath = $OPEN_TABS_SELECTED_FILEPATH.replace(/\\/g, "/");
    ...
    if (LanguageId === null) {
      ...
      OUTPUTS.update((outputs) => [
        ...outputs,
        {
          ...
          status: "error",
          output: "N/A",
          error: [`Language '${LanguageId}' cannot be compiled.`], // Corrected: Wrapped in array
        },
      ]);
      setTimeout(() => {
        statusMessage = "";
    ...
    try {
      ...
      console.log("comp: ", result.compile_output);
      ...
      console.log("Compliation: ", $OUTPUTS);
...
<div class="out-cnt">
  <div class="out-table">
    {#each $OUTPUTS as output}
      <div class="out-info">
        <span style="color: grey;">{output.timestamp}</span>
        <span style={output.status === "error" ? "color:red;" : "color:green;"}
          >{output.status}</span
        >
        <span style="color: grey;">{output.fileName}</span><br />
      </div>
      {#if output.error !== undefined}
        <span>{">"}{output.error}</span><br />
      {/if}
