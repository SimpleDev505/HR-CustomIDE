<script lang="ts">
  ...
  async function LoadTab_Content() {
    try {
      ...
      const newModel = editor.createModel(editor_content, languageId);
  ...
  onMount(() => {
    ...
    $EDITOR_AREA = editor.create(container, {
      ...
      theme: "vs-dark",
      fontSize: 20,
      lightbulb: editor.ShowLightbulbIconMode.On,
    });
  ...
  $effect(() => {
    if ($EDITOR_AREA && $EDITOR_AREA.getValue() !== editor_content) {
      $EDITOR_AREA.setValue(editor_content);
    }
