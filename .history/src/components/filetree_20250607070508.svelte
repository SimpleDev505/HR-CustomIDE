            ...
            onmousedown={() => {
              if (!renaming && !file.is_dir) {
                OPEN_TABS.update((tabs) => {
                  ...
                  const file_index = tabs.findIndex(
                    (tab) => tab.tab_name === file.name
                  );
                  $OPEN_TABS_CURRENT_SELECTED = file_index;
                  console.log($OPEN_TABS_CURRENT_SELECTED);
                  if ($OPEN_TABS_CURRENT_SELECTED != -1) {
                    $OPEN_TABS_SELECTED_FILEPATH = tabs[file_index].tab_path;
                    console.log($OPEN_TABS_SELECTED_FILEPATH);
                  }
                  if (!file_exists) {
                    return [
                      ...tabs,
