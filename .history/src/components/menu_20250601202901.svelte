<script lang="ts">
  ...
  let current_selectedoption: HTMLElement;
  let dropdown_file = $state(false);
  let dropdown_edit = $state(false);
  let dropdown_settings = $state(false);
  let dropdown_run = $state(false);
  function clickOption(name: string, event: MouseEvent) {
    current_selectedoption = event.currentTarget as HTMLElement;
    switch (name) {
      case "file":
        dropdown_file = true;
        //
...
<div class="menu-cnt">
  <div class="menu-tools">
    <span>HR</span>
    <ul class="menu-nav">
      <li><button onclick={clickOption}>File</button></li>
      <li>Edit</li>
      <li>Settings</li>
      <li>Run</li>
    </ul>
