<script lang="ts">
  ...
  let currentselected_file: HTMLElement;
      ...
      {#if $APP_STATE && $APP_STATE.file_tree.length > 0}
        {#each $APP_STATE.file_tree as file}
          <div
            role="button"
            tabindex="999"
            onmousedown={() => setSelectedFile(file.path)}
          ...
          >
            ...
            >
              ...
              >
                {#if !rename_file && file.path != $SelectedFile_Path}
                  {file.name}
                {:else if file.path === $SelectedFile_Path}
                  <input type="text" />
                {/if}
              </div>
            </span>
