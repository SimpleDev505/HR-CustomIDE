<script lang="ts">
  ...
  let currentLanguageExtension: LanguageSupport | Extension[] = python();
  //Styles
  ...
  function setLanguageExtension() {
    if ($OPEN_TABS_SELECTED_FILEPATH) {
      const fileExtension = $OPEN_TABS_SELECTED_FILEPATH
        .split(".")
        .pop()
        .toLowerCase();
      let newExtension; // This will hold the CodeMirror language extension function
  ...
  onMount(() => {
    ...
    const state = EditorState.create({
      ...
      extensions: [
        basicSetup,
        comp.of(currentLanguageExtension),
        oneDark,
        autocompletion({
