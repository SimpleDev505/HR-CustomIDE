<script lang="ts">
  ...
  export let minimumHeightPx: number = 50; // Initial height in pixels
  export let maximumHeightPx: number = 100; // Initial height in pixels
  let className = "";
  ...
  function handleResize(event: MouseEvent) {
    ...
    const minHeight = minimumHeightPx; // pixels
    currentHeight = Math.max(minHeight, newHeight);

    containerElement.style.height = `${currentHeight}px`; // Apply directly
    containerElement.style.minHeight = `${minimumHeightPx}px`;
    containerElement.style.maxHeight = `${maximumHeightPx}px`;
    containerElement.style.width = `100%`;
    // containerElement.classList.add(addclass);
  }
...
<div
  ...
  class="resizable-vertical {$$props.class}"
...
<style>
  ...
  .resizer-handle {
    position: absolute;
    left: 0;
    height: 3px; /* Thickness of the handle */
    width: 100%;
