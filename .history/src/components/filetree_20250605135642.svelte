<script lang="ts">
  ...
  //Filetree Menu
  let filetree_menu_pos: number = $state(0);
  let enable_filetree_menu = $state(false);
  ...
  onMount(() => {
    document
      .getElementById("filetree-cnt")
      ?.addEventListener("click", (e: MouseEvent) => {
        e.preventDefault();
        filetree_menu_pos = e.clientX;
        enable_filetree_menu = true;
      });
  });
</script>
...
>
  <div id="filetree-cnt">
    ...
    {#if enable_filetree_menu}
      <Dropdownmenu leftpx={filetree_menu_pos} items={filetree_menu_items}
      ></Dropdownmenu>
    {/if}
  </div>
</Resizerhorizontal>
