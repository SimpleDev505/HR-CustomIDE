<script lang="ts">
  ...
  async function SaveFile() {
    ...
    const result = await invoke("save_file_command", {
      path: $OPEN_TABS_SELECTED_FILEPATH,
      contents: _editor_content,
    });
  }
  function CloseTab(fileno: number) {
    let next_file = fileno + 1;
    const wasActiveTabClosed = fileno === $OPEN_TABS_CURRENT_SELECTED;
    if ($OPEN_TABS.length === 0) {
      _editor_content = "";
      $OPEN_TABS_CURRENT_SELECTED = -1;
      current_tab_content = "";
      editor_path_formated = "";
    } else if (wasActiveTabClosed) {
      $OPEN_TABS_CURRENT_SELECTED = Math.min(fileno, $OPEN_TABS.length - 1);
    }
    // } else if (fileno < $OPEN_TABS_CURRENT_SELECTED) {
    //   $OPEN_TABS_CURRENT_SELECTED--;
    // }
    // if (next_file > $OPEN_TABS.length) {
    // } else {
    //   $OPEN_TABS_CURRENT_SELECTED = next_file;
    // }

    let del = $OPEN_TABS.splice(fileno, 1);
    $OPEN_TABS = $OPEN_TABS; //Rerender With New tabs
    console.log("Deleted : ", del, "New tabs: ", $OPEN_TABS);
  }
  onMount(() => {
  ...
  $effect(() => {
    ...
    $EDITOR_AREA.onDidChangeModelContent(() => {
      _editor_content = $EDITOR_AREA.getValue();
    });
...
<div class="editor-core-layout">
  <div class="editor-core-opentabs">
    ...
    {#each $OPEN_TABS as tab, tab_index}
      ...
      >
        {tab.tab_name}
        <button id="tab-close" onclick={() => CloseTab(tab_index)}>x</button>
      </span>
    {/each}
  </div>
