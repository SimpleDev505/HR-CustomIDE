...
async fn awake_filewatcher(app_handle: AppHandle, project_path: String) -> Result<(), String> {
    ...
    let watcher_result = RecommendedWatcher::new(
        tx,
        notify::Config::new().with_poll_interval(Duration::from_secs(2)),
    );
                    ...
                    match res {
                        Ok(event) => {
                            let should_rescan = match event.kind {
                                ...
                                EventKind::Modify(m) => {
                                    if m.contents || m.name {
                                        true
                                    } else {
                                        event.paths.iter().any(|p| {
                                            p.file_name()
                                                .map_or(false, |name| name != "project.hre")
                                        })
                                    }
                                }
                                EventKind::Any => true,
                            ...
                            if should_rescan {
                                ...
                                if let Err(e) = rescan_result {
                                    ...
                                    let _ = app_handle_clone
                                        .emit_all("app_state_updated", current_state);
                                }
                            }
                        }
