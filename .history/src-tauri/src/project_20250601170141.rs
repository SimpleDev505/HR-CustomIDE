...
pub fn create_project() -> Result<ProjectMetadata, String> {
    // Get home directory or fallback to temp directory
    let base_path = dirs::home_dir()
        .unwrap_or_else(|| std::path::PathBuf::from(std::env::temp_dir()))
        .to_str()
        .ok_or("Failed to resolve base directory")?
        .to_string();
    ...
    // Write placeholder content to the .hre file
