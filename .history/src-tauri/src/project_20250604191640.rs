...
pub fn open_project(app_handle: AppHandle) {
    ...
    let project_path = tauri::api::dialog::blocking::FileDialogBuilder::new() // Use tauri::api::dialog::blocking for FileDialog
        .set_directory(std::env::var("HOME").unwrap_or_default())
        .pick_folder();
    ...
    // --- FIX START ---
    // Ensure PROJECT_LOADED is defined and accessible, assuming it's a global Mutex<bool>
    // If PROJECT_LOADED is not defined, you'll need to add it like APP_STATE.
    // For now, I'll assume it exists and is used as a flag.
    // If it's a global static, it should be accessed like APP_STATE.
    // I'll add a placeholder for PROJECT_LOADED if it's not defined in the full context.
    lazy_static! {
        pub static ref PROJECT_LOADED: Mutex<bool> = Mutex::new(false);
    }
    ...
    let mut state = APP_STATE.lock().unwrap(); // Acquire lock on APP_STATE
    let current_project_path = project_path.clone(); // Clone project_path for load_state
    let loaded_state_result = load_state(&current_project_path); // Call load_state with cloned path

    let final_state: AppState; // Declare final_state to hold the result

    if let Ok(loaded_state) = loaded_state_result {
        // If state was loaded successfully from .hre
        *state = loaded_state;
        println!(
        ...
        *loaded_guard = true;
        final_state = state.clone(); // Clone the state for emitting and watcher
        drop(state); // Drop the mutex guard early
    } else {
        // If no project.hre or error loading, scan the folder
        *loaded_guard = false;
        ...
        // Call scan_project_directory to get the updated state
        let scanned_state = match scan_project_directory(project_path.clone()) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("Error scanning project directory: {}", e);
                // Emit error to frontend if scan fails
                let _ =
                    app_handle.emit_all("backend_error", format!("Failed to open project: {}", e));
                return; // Return if scanning fails
            }
        };
        *state = scanned_state.clone(); // Update global APP_STATE
        final_state = scanned_state; // Use the scanned state for emitting and watcher
        drop(state); // Drop the mutex guard early

        println!("Opened project '{}' at {:?}", project_name, project_path);
    }

    // Emit event to frontend after initial load/scan
    let _ = app_handle.emit_all("app_state_updated", final_state.clone());

    // Clone app_handle for the spawned task
    let app_handle_clone_for_spawn = app_handle.clone();
    let project_path_for_watcher = final_state
        .project_path
        .clone()
        .unwrap() // We know project_path is Some here after successful load/scan
        .to_string_lossy()
        .into_owned();

    // Spawn awake_filewatcher in a separate async task
    tauri::async_runtime::spawn(async move {
        match awake_filewatcher(app_handle_clone_for_spawn, project_path_for_watcher).await {
            Ok(_) => println!("[open_project] File watcher spawned successfully."),
            Err(e) => eprintln!("[open_project] Error spawning file watcher: {}", e),
        }
    });
}
