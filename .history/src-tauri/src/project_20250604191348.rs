...
#[tauri::command]
pub fn open_project(app_handle: AppHandle) -> Result<AppState, String> {
    // Prompt the user to select a directory
    let selected_path = tauri::api::dialog::blocking::FileDialogBuilder::new()
        .pick_folder()
        .ok_or("No project directory selected.".to_string())?;

    let project_path = PathBuf::from(selected_path.to_string_lossy().into_owned());

    // Load or scan the project initially
    let loaded_state_result = load_state(&project_path);

    let final_state = match loaded_state_result {
        Ok(state) => {
            ...
            if state.project_path.is_none() {
                ...
                *app_state_guard = state;
            }
            let updated_state = app_state_guard.clone();
            drop(app_state_guard);
            updated_state
        }
        Err(e) => {
            eprintln!("Error loading .hre file: {}. Will rescan directory.", e);
            scan_project_directory(project_path.clone())?
        }
    };

    // Emit event to frontend after initial load/scan
    app_handle
        .emit_all("app_state_updated", final_state.clone())
        .map_err(|e| format!("Failed to emit app_state_updated event: {}", e))?;

    // Clone app_handle and project_path for the spawned task
    ...
    tauri::async_runtime::spawn(async move {
        let _ = awake_filewatcher(app_handle_clone_for_spawn, project_path_for_watcher).await;
    });
