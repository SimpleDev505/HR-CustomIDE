...
fn scan_project_directory(project_root: PathBuf) -> Result<AppState, String> {
    ...
    // `min_depth(1)` ensures we don't include the root directory itself in the file_tree.
    for entry in WalkDir::new(&project_root).min_depth(1) {
        ...
        // --- CHANGE START ---
        // Store the full absolute path directly.
        ...
        let name = path
            .file_name()
            .and_then(|n| n.to_str())
            .unwrap_or_default() // Use a default empty string if name cannot be converted to string.
            .to_string();
        ...
        let language = if is_dir {
            ...
            path.extension()
                ...
                .map(|s| s.to_lowercase()) // Convert extension to lowercase.
                .unwrap_or("text".to_string()) // Default to "text" if no extension.
        };

        // Create a new FileNode and add it to the file_tree.
        file_tree.push(FileNode {
            name,
            path: full_abs_path, // --- CHANGE: Store the full absolute path here ---
            is_dir,
            language,
        });
    }
