...
pub fn open_project(app_handle: AppHandle) {
    ...
    if let Some(loaded_state) = load_state(&project_path) {
        ...
        *loaded_guard = true;
        let final_state = state.clone(); // Clone the state for emitting
        let project_path_for_watcher_str = final_state
            .project_path
            .clone()
            .unwrap()
            .to_string_lossy()
        ...
        let app_handle_clone_for_spawn_if = app_handle.clone();
        // Explicitly drop the loaded_guard before spawning the async task
        drop(loaded_guard);
        drop(state);
        // println!("Opened project Signal '{:?}'", app_state_trigger);
        tauri::async_runtime::spawn(async move {
            // Spawn awake_filewatcher
            match awake_filewatcher(app_handle_clone_for_spawn_if, project_path_for_watcher_str)
                .await
            {
    ...
    *loaded_guard = true;
    ...
    tauri::async_runtime::spawn(async move {
        // Spawn awake_filewatcher
        match awake_filewatcher(app_handle, projectpath).await {
            Ok(_) => println!("[open_project] File watcher spawned successfully."),
            Err(e) => eprintln!("[open_project] Error spawning file watcher: {}", e),
        }
    });
...
//Save File on Editor(ctrl s)
#[tauri::command]
pub async fn save_file_command(path: String, contents: String) -> Result<bool, String> {
    let final_path = PathBuf::from(&path); // Start with the path as provided
    ...
    match fs::write(&final_path, contents) {
        Ok(_) => {
            println!("Successfully saved file: {:?}", final_path);
            Ok(true) // Return true on success
        }
        Err(e) => {
            eprintln!("Failed to save file {:?}: {}", final_path, e);
            // Return an error string for more detailed feedback to the frontend
